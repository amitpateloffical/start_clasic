...
const Aircoolers = () => {
  return (
    <div className="w-full min-h-screen bg-gradient-to-br from-gray-50 to-white px-4 pt-28 pb-10 flex flex-col items-center">
      {categories.map(({ key, label }) => {
        const items = products[key] || [];

        if (!items.length) return null; // Skip if no products in this category

        return (
          <div key={key} className="mb-16 w-full max-w-5xl">
            {/* Category Title */}
